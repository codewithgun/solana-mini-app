...
impl Pack for GameInfo {
    ...
    // Unpack account data to GameInfo
    fn unpack_from_slice(src: &[u8]) -> Result<Self, ProgramError> {
        ...
        let (is_initialized, spl_token_account) = array_refs![&src, 1, 32];
    ...
    // Pack GameInfo struct into account data
    fn pack_into_slice(&self, dst: &mut [u8]) {
        ...
        let (is_initialized_dst, spl_token_account_dst) = mut_array_refs![dst, 1, 32];
